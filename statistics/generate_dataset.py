...
def generate_class():
    ...
    Returns:
        str: Una classe tra 'Mago', 'Guerriero', 'Ladro'.
    """
...
def generate_cluster():
    ...
    Returns:
        str: Uno tra 'Casual', 'Hardcore', 'Spender'.
    """
...
def generate_acquisition():
    ...
    Returns:
        str: Uno tra 'Pubblicità', 'Referral', 'Organic'.
    """
...
def generate_ora_punta():
    ...
    Returns:
        int: Ora del giorno (0-23).
    """
    weights = [5, 3, 2, 2, 1, 1, 2, 5, 8, 10, 12, 10, 8, 7, 6, 4, 4, 4, 3, 2, 2, 1, 1, 1]
...
def generate_items():
    ...
    Returns:
        str: Stringa con uno o più oggetti separati da virgola.
    """
...
# Creazione dizionario con i dati
data = {
...
# -----------------------
# Inserimento valori mancanti per simulare preprocessing
# -----------------------
# Percentuali realistiche di missing
missing_numeric = ['eta', 'salute_media', 'attacco_medio', 'crediti_attuali', 'spesa_mensile']
missing_categorical = ['genere', 'istruzione', 'classe_personaggio']

for col in missing_numeric:
    df.loc[df.sample(frac=0.05).index, col] = np.nan  # 5% valori mancanti numerici

for col in missing_categorical:
    df.loc[df.sample(frac=0.03).index, col] = np.nan  # 3% valori mancanti categorici
